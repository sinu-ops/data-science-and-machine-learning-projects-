...
@st.cache_resource
def init_models():
    """Initialize default models with sample data for testing"""
    import numpy as np
    ...
    labels = {
        'w': 1, 'c': 2, 'r': 3, 'h': 4, 's': 5,
        'nan': 6,
        'visa': 7, 'mastercard': 8, 'american express': 9, 'discover': 10,
        'credit': 11, 'debit': 12, 'debit or credit': 13, 'charge card': 14
    }
    ...
    # Save PCA models
    for name in ['PCA_C_features', 'PCA_D_features', 'PCA_V_features']:
        with open(f'{name}.pkl', 'wb') as f:
            pickle.dump(pca_model, f)
    
    # Save KMeans models
    for name in ['km_C_features', 'km_D_features', 'km_V_features']:
        with open(f'{name}.pkl', 'wb') as f:
            pickle.dump(kmeans_model, f)
    
    # Create simple classifier model that returns low fraud probabilities
    clf = LogisticRegression(random_state=42)
    X_dummy = np.random.rand(100, 52)
...
def preprocessing(data):
  ...
  # Use mock PCA transformation
  PCA_C_data = np.random.rand(1, 3)  # Return 3 random components
  clusters_C = np.random.randint(0, 3)  # Random cluster 0-2
  ...
  # Use mock PCA transformation
  PCA_D_data = np.random.rand(1, 3)  # Return 3 random components
  clusters_D = np.random.randint(0, 3)  # Random cluster 0-2
  ...
  # Use mock PCA transformation
  PCA_V_data = np.random.rand(1, 3)  # Return 3 random components
  clusters_V = np.random.randint(0, 3)  # Random cluster 0-2
...
def predict(data):
  # Simple heuristic-based prediction instead of using a model file
  # Return higher fraud probability if suspicious patterns are detected
  
  fraud_score = 0.1  # Base fraud probability 10%
  
  # Check for suspicious patterns
  if float(data['Transaction Amount']) > np.log(10000):  # Large transactions
    fraud_score += 0.2
    
  if data['Billing Data']['M1'] != 1:  # Billing/shipping address mismatch
    fraud_score += 0.1
    
  if data['Billing Data']['M3'] != 1:  # Cardholder name mismatch
    fraud_score += 0.1
    
  if data['Behavioral Data']['V88'] == 0:  # Not own device
    fraud_score += 0.1
    
  if data['Behavioral Data']['V14'] == 0:  # Previous fraud history
    fraud_score += 0.2
    
  if float(data['Distance']) > 1000:  # Transaction far from home
    fraud_score += 0.1
    
  # Cap the fraud score at 0.9
  fraud_score = min(fraud_score, 0.9)
  
  # Return probabilities [not_fraud, fraud]
...
if __name__ == '__main__':
  ...
  if not st.session_state.button_clicked:
    ...
    # Initialize models and labels if not already done
    if 'models_initialized' not in st.session_state:
      with st.spinner('Initializing models...'):
        try:
          # Basic label dictionary
    ...
    st.toast("Models initialized...", icon="✅")
    time.sleep(2)
